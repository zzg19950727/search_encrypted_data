...
功能：向给定的数据加盐
入口：
unsigned char *input    需要加盐的数据
int ilen                需要加盐的数据的长度
返回：
char *             加盐后的数据output = salt+input
*/
unsigned char *addSalt(unsigned char *input,int ilen)
{
    char salt[32] = {0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10,
                     0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};

    unsigned char *output = (unsigned char *)malloc((ilen+32)*sizeof(unsigned char));

    memcpy(output,salt,32);
    memcpy(output+32,input,ilen);
